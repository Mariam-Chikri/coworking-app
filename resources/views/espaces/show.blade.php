...
<div class="cw-page-header">
    <div class="cw-container">
        <div style="display:flex;align-items:center;gap:1rem">
            <a href="{{ route('espaces.index') }}" style="color:rgba(255,255,255,.7);transition:var(--transition)" onmouseover="this.style.color='white'" onmouseout="this.style.color='rgba(255,255,255,.7)'">
...
<div class="cw-container" style="padding-bottom:4rem">
    <div style="display:grid;grid-template-columns:1fr 380px;gap:2rem;align-items:start">
        ...
        <div>
            {{-- Image --}}
            <div style="height:320px;background:{{ $espace->couleur ?? 'var(--gradient)' }};border-radius:var(--radius-lg);display:flex;align-items:center;justify-content:center;font-size:6rem;color:white;margin-bottom:2rem;overflow:hidden">
            ...
            <div style="background:white;border-radius:var(--radius-lg);padding:2rem;margin-bottom:1.5rem;box-shadow:var(--shadow-sm)">
                <div style="display:flex;gap:1.5rem;flex-wrap:wrap;margin-bottom:1.5rem">
                    <div style="display:flex;align-items:center;gap:.6rem;font-size:.95rem">
                        ...
                        {{ $espace->capacite }} {{ __('messages.personnes_max') }}
                    </div>
                    <div style="display:flex;align-items:center;gap:.6rem;font-size:.95rem">
                        <i class="fas fa-tag" style="color:var(--primary)"></i>
                        {{ __('messages.'.$espace->type) }}
                    </div>
            ...
            <div style="background:white;border-radius:var(--radius-lg);padding:2rem;box-shadow:var(--shadow-sm)" id="avis">
                <h3 style="font-weight:700;margin-bottom:1.5rem">
                    ...
                    {{ app()->getLocale() === 'en' ? 'Reviews' : 'Avis' }}
                </h3>
                @if($espace->avis->isEmpty())
